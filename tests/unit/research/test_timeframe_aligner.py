...
def test_point_in_time_filtering_excludes_future_and_open_candles():
    ...
    candles = [
        make_candle(0, 100.0),
        make_candle(3600000, 101.0),
        make_candle(7200000, 102.0)
    ]
    ...
    visible = TimeframeAligner.filter_visible_candles(candles, decision_timestamp=5000000, timeframe="1H")
    assert len(visible) == 1
    assert visible[0].timestamp == 0

    # At decision time T = 7,200,000:
    # Candle 1 and Candle 2 have closed
    visible = TimeframeAligner.filter_visible_candles(candles, decision_timestamp=7200000, timeframe="1H")
    assert len(visible) == 2
    assert [c.timestamp for c in visible] == [0, 3600000]
