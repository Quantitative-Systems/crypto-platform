...
HYPOTHESIS H_D03_DEV:
- When active trade price causally reaches >= +1.0R excursion, protective stop is moved to Entry + 0.10R.
...
def run_single_stream_dev(asset: str, tf_set_id: str) -> Dict[str, Any]:
    ...
    replayer = CausalReplayer(
        ...
        enable_profit_lock=True,  # ISOLATED D-03 ACTIVATION
        profit_lock_trigger_r=1.0,# Move stop when causally reaching +1.0R
        profit_lock_stop_r=0.10,  # Lock Entry + 0.10R
...
def run_full_dev_matrix():
    print("=" * 80)
    print("STARTING ISOLATED H_D03_DEV MATRIX REPLAY (2021-01-01 to 2022-12-31)")
    print("=" * 80)
    ...
    dev_results_payload = {
        "metadata": {
            "title": "H_D03_DEV Isolated Hypothesis Development Replay",
            "hypothesis_id": "H_D03_DEV",
            "description": "Post-entry Breakeven +0.10R lock upon causally reaching +1.0R excursion. All other canonical rules preserved.",
            "temporal_partition": "DEVELOPMENT (2021-01-01 to 2022-12-31)",
            "timestamp": datetime.now(timezone.utc).isoformat(),
            "git_commit": "e9cc4b7",
            "rules": {
                "profit_lock_trigger_r": 1.0,
                "profit_lock_stop_r": 0.10,
    ...
    print(f"\nSuccessfully written H_D03_DEV results to {out_file}")
