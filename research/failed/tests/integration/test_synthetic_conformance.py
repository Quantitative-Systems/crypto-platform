...
def make_payload(
    ...
    return MarketStatePayload(
        ...
        events=[], swings=[], structure_state=struct, liquidity_pools=[],
        keyzones=[KeyZone(
            zone_id=f"KZ_{timeframe}_{timestamp}",
            zone_type=KeyZoneType.BULLISH_OB if trend == TrendDirection.BULLISH else KeyZoneType.BEARISH_OB,
            scope=ZoneScope.EXTERNAL,
            price_level=current_price,
            high_boundary=current_price + 2.0,
            low_boundary=current_price - 2.0,
            creation_timestamp=timestamp - 100,
