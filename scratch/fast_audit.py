...
# Manually override the loop to break early
original_run = replayer.run

def monkey_patched_run(*args, **kwargs):
    # This is too complex to monkey patch the loop easily, but wait!
    # I can just monkey patch TradeLedger.close_trade to print and exit!
    original_close = replayer.ledger.close_trade
    def hooked_close(*c_args, **c_kwargs):
        trade = original_close(*c_args, **c_kwargs)
        if trade:
            print("="*60)
            print("CATASTROPHIC TRADE FOUND!")
            import json
...
replayer.run = monkey_patched_run
replayer.run(
    symbol=symbol,
