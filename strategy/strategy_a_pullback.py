...
from market_intelligence.primitives import MarketStatePayload, TrendDirection, KeyZone, EventType
...
class StrategyAPullbackEngine:
    ...
    def evaluate_pullback_setup(
        ...
        2. MTF trend must have completed its structural realignment (CHOCH/BOS) back into HTF Bias direction.
        3. Active unmitigated MTF Keyzone present.
        """
        ...
        # Rule 1: HTF must be in PULLBACK phase (Recent BOS)
        if not htf_state.last_event or htf_state.last_event.event_type != EventType.BOS:
            return StrategyAResult(
                ...
                reason="HTF is not in PULLBACK phase (No recent BOS)."
            )
        ...
        if mtf_state.trend != htf_state.trend:
            return StrategyAResult(
                is_valid_setup=False, mtf_keyzone=None,
                reason=f"MTF trend ({mtf_state.trend.value}) has not realigned with HTF bias ({htf_state.trend.value})."
            )
        ...
        if not mtf_state.last_event or mtf_state.last_event.direction != htf_state.trend:
            return StrategyAResult(
                is_valid_setup=False, mtf_keyzone=None,
                reason="MTF structural realignment CHOCH/BOS missing."
            )
        ...
        if not matching_zones:
            return StrategyAResult(
                is_valid_setup=False, mtf_keyzone=None,
                reason="No unmitigated MTF Keyzone found for Pullback entry."
            )
