...
# Override MockSwingCache to print!
class NoisyMockSwingCache(deterministic_fixtures.MockSwingCache):
    def get_target(self, idx, direction):
        print(f"get_target called with self.target={self.target}")
        return self.target

deterministic_fixtures.MockSwingCache = NoisyMockSwingCache
